...
module Ctx2d1 = struct
  ...
  let save (_, ctx) = ctx##save
  let restore (_, ctx) = ctx##restore
  let rotate (_, ctx) angle = ctx##rotate (Js_of_ocaml.Js.number_of_float angle)

  let translate (_, ctx) ~x ~y =
    ctx##translate (Js_of_ocaml.Js.number_of_float x) (Js_of_ocaml.Js.number_of_float y)
  ;;
end
