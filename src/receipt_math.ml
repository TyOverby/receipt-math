...
(* The generated program is either an integer or a floating-point expression; [Param.numeric]
   decides which one this run uses. *)
type expr =
  | Int of Expr_int.t
...
(* Evaluate the program into a square float grid of values in [0, 1] (row-major), returning
   the grid together with its side length. The whole pipeline downstream works on this float
   buffer rather than on bytes, so values keep full floating-point precision until the final
   write to a canvas.

   The two evaluators are sampled differently, returning the [`filter] to upscale with:
   - [Expr_int] is modular byte art defined on an integer grid of coordinates in [0, 255], so
     it is rendered at its native [scaled] resolution and upscaled with [`nearest] to keep the
     characteristic blocky pixels.
...
(* Linearly stretch the grid so its values span the full [0, 1] range. Done in floating point
   to avoid the banding that a 256-level byte round-trip would introduce. *)
let tonemap grid =
  ...
  if Float.equal range 0. then grid else Array.map grid ~f:(fun v -> (v -. min_value) /. range)
;;

(* Sample a [size]x[size] grid at continuous source coordinates [fx]/[fy], clamping at the
   edges. [`nearest] snaps to the closest cell (blocky upscaling, used for the integer art);
   [`bilinear] interpolates the four neighbours, kept in floating point so the colour ramp
   below sees a smooth signal rather than a 256-level one. *)
...
(* Map a source grid coordinate onto the [size]x[size] grid for an output pixel [i] of an
   [out]-wide image, using pixel-centre alignment. When [size = out] (the float path) this is
   the identity, so no interpolation happens. *)
...
(* Render the colourised [out]x[out] output directly from the float grid: for each output
   ...
   map it through the Oklab colour ramp. Only the final Oklab->rgb step quantises to bytes,
   so the gradient is smooth instead of stepping through 256 grey levels. *)
...
(* Render the [out]x[out] greyscale preview from the float grid. The preview is displayed as
   bytes regardless, so quantising the grey level here costs nothing. *)
